...
var arraySum = function(array) {
	...
	//array[0] is an array:
	//  call recursion(array[0])+recursion(array.slice(1));
...
var exponent = function(base, exp) {
	...
	else if (exp < 1) return 1/exponent(base, -exp);
};
...
var powerOfTwo = function(n) {
	if (n===1) return true;
	else if (n/2 >= 1)  return powerOfTwo(n/2);
	else return false;
};
...
var reverse = function(string) {
	/* possibilities:
		string.length > 1:
			return string(max)+reverse(string_without_max)
		string.length === 1:
			return string;
	*/
...
var modulo = function(x, y) {
	/* possibilities:
		1) (x-y) === 0: return 0;
		2) x-2y > 0: return modulo(x-y,y)
		3) x-y > 0: return x-y;
	*/
	if (y === 0) return NaN;
	else if (x < 0 && y < 0) return -modulo(-x,-y);
	else if (x < 0) return -(modulo(-x,y));
	else if (y < 0) return -(modulo(x,-y));
	else if (y > x) return x;
	else if (x-y === 0) return 0;
	else if (x-y > 0) return modulo(x-y,y);
};
